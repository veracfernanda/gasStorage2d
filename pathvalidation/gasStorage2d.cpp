...
T pressureDrop = 2000.;                       // pressure drop [N.m-2]

const T densityGas = 1.204;                      // gas density [kg.m-3]
const T densityLiquid = 791.2;                   // liquid density [kg.m-3]

const T viscosityGas = 1.517e-5;                // gas kinematic viscosity [m2.s-1]
const T viscosityLiquid = 1.520e-6;              // liquid kinematic viscosity [m2.s-1]

const T surfaceTension = 22.36e-3;                // surface tension [N.m-1]

const T contactAngle = M_PI * 40. / 180.;      // Contact angle [radians]

const T maxPhysTime  = 0.1;                  // max simulation time [s]
const T vtkIter      = 0.00005;              // write simulation output [s]
const T statIter     = 0.00005;              // simulation statistics time [s]
                    ...
                    SuperGeometry<T,2>& superGeometry)
{
  ...
  AnalyticalIdentity2D<T, T> phi0_rev(one - phi0);

  AnalyticalIdentity2D<T, T> rho0(rhov + (rhol - rhov) * phi0_rev);
  AnalyticalIdentity2D<T, T> tau0(tauv + (taul - tauv) * phi0_rev);
  ...
  sLatticeAC.defineRhoU(all, phi0_rev, u0);
  sLatticeAC.iniEquilibrium(all, phi0_rev, u0);
...
int main(int argc, char* argv[])
{
  ...
  singleton::directories().setOutputDir("/hpcwork/tr420207/ITV-Seminar/01-Run/validation2000-v2/");
